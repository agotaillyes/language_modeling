...
def ngram_unsmoothing_prob(i,ngram_letter_counter,n_1gram_letter_counter,tokens_nr):
    ...
    if i == 1:
        ...
        for ngram,value in ngram_letter_counter.iteritems():
            ...
            last_word=list[1]
            #print first_word
            #print n_1gram_letter_counter[first_word]
            if n_1gram_letter_counter[first_word] != 0:
                #print value
                #print n_1gram_letter_counter[first_word]
                ngram_prob_list[ngram] = (value+0.0) / n_1gram_letter_counter[first_word]
...
if __name__ == '__main__':
    ...
    tokens_nr_train = sum(train_token_list.values())
...
##    print sorted(unigram_word_counter_train.iteritems(),key=lambda (k,v):v,reverse=True)
    ...
    bigram_unsmoothing_prob = ngram_unsmoothing_prob(2,bigram_word_counter_train,unigram_word_counter_train,tokens_nr_train)
    
    train_unigram_unsmoothing = train_word_ngram(bigram_unsmoothing_prob) 
    ranked_train = convert_train_to_rank(train_unigram_unsmoothing)  
    
    test_unigram_unsmoothing = test_part(bigram_word_counter_test)  
    
    all_test_bigrams_nr = sum(bigram_word_counter_test.values())
    
    result= result_list(test_unigram_unsmoothing,ranked_train,all_test_bigrams_nr)
##    print all_train_bigrams_nr
