...
def c_star(order,ngram_letter_counter,ngram_witten_bell,n_1gram_letter_counter):
    ...
    for ngram,value in ngram_letter_counter.iteritems():
        if value != 0:
            c_star[ngram]=round(n_1gram_letter_counter[ngram[0:order-1]]*ngram_witten_bell[ngram])
        else:
            alma=n_1gram_letter_counter[ngram[0:order-1]]*ngram_witten_bell[ngram]
            if alma >= 0.5:
                c_star[ngram]=round(alma)
            else:
                c_star[ngram]=n_1gram_letter_counter[ngram[0:order-1]]*ngram_witten_bell[ngram]
    return c_star
...
def bigram_backoff(bigram_letter_counter,unigram_letter_counter,unigram_prob_tilde,prob_tilde,alpha_n_1gram,all_letter_nr):
    backoff_prob_list = collections.defaultdict(int)
    alma = collections.defaultdict(int)
    ...
    for ngram,value in bigram_letter_counter.items():
        if value > 0:
            ...
            backoff_prob_list[ngram]=alpha_n_1gram * unigram_prob_tilde[ngram[1:]]
...
if __name__ == '__main__':
    ...
    bigram_witten_bell1 = ngram_witten_bell_prob(2,bigram_letter_counter1,unigram_letter_counter1,letter_types_nr1)
    bigram_witten_bell2 = ngram_witten_bell_prob(2,bigram_letter_counter2,unigram_letter_counter2,letter_types_nr2)
    bigram_witten_bell3 = ngram_witten_bell_prob(2,bigram_letter_counter3,unigram_letter_counter3,letter_types_nr3)
    bigram_witten_bell4 = ngram_witten_bell_prob(2,bigram_letter_counter4,unigram_letter_counter4,letter_types_nr4)
    
    bigram_star1 = c_star(order,bigram_letter_counter1,bigram_witten_bell1,unigram_letter_counter1)
    bigram_star2 = c_star(order,bigram_letter_counter2,bigram_witten_bell2,unigram_letter_counter2)
    bigram_star3 = c_star(order,bigram_letter_counter3,bigram_witten_bell3,unigram_letter_counter3)
    bigram_star4 = c_star(order,bigram_letter_counter4,bigram_witten_bell4,unigram_letter_counter4)
    ...
    bigram_backoff_prob1=bigram_backoff(bigram_letter_counter1,unigram_letter_counter1,unigram_prob_tilde1,bigram_prob_tilde1,unigram_alpha1,tokens_nr1)
    bigram_backoff_prob2=bigram_backoff(bigram_letter_counter2,unigram_letter_counter2,unigram_prob_tilde2,bigram_prob_tilde2,unigram_alpha2,tokens_nr2)
    bigram_backoff_prob3=bigram_backoff(bigram_letter_counter3,unigram_letter_counter3,unigram_prob_tilde3,bigram_prob_tilde3,unigram_alpha3,tokens_nr3)
    bigram_backoff_prob4=bigram_backoff(bigram_letter_counter4,unigram_letter_counter4,unigram_prob_tilde4,bigram_prob_tilde4,unigram_alpha4,tokens_nr4)
...
######################################## TEST PART ###########################################
    ...
    for ngram,value in bigram_letter_counter_test.iteritems():
        ...
        for word in ngram.split():
            actual_list[word]+=1
        result1=test_part(actual_list,order,bigram_backoff_prob1)
        result2=test_part(actual_list,order,bigram_backoff_prob2)
        result3=test_part(actual_list,order,bigram_backoff_prob3)
        result4=test_part(actual_list,order,bigram_backoff_prob4)
...
##    print sorted(bigram_backoff_prob1.iteritems(), key=lambda (k,v):v,reverse=True)