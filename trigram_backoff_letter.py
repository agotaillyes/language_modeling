...
def c_star(i,ngram_letter_counter,ngram_witten_bell,n_1gram_letter_counter):
    c_star = collections.defaultdict(int)
    
    for ngram in ngram_witten_bell:
        if ngram_letter_counter[ngram] != 0:
            c_star[ngram]=round(n_1gram_letter_counter[ngram[0:i-1]]*ngram_witten_bell[ngram])
        else:
            alma=n_1gram_letter_counter[ngram[0:i-1]]*ngram_witten_bell[ngram]
            if alma >= 0.5:
...
def alpha(prob_tilde_list,prob_plus1tilde_list,ngram_letter_counter,n_plus1_gram_letter_counter):
    ...
    for ngram in n_plus1_gram_letter_counter:
        if ngram_letter_counter[ngram[0:len(ngram)-1]] > 0:
            sum_counter += prob_plus1tilde_list[ngram]
            sum_denominator += prob_tilde_list[ngram[1:]]
    alpha = (1.0-sum_counter)/(1-sum_denominator)
...
def trigram_backoff(trigram_letter_counter,bigram_letter_counter,unigram_letter_counter,prob_tilde,prob_1tilde,bigram_alpha,unigram_alpha,tokens_nr):
    ...
    for ngram, value in prob_tilde.items():
        if trigram_letter_counter[ngram] > 0:
            trigram_backoff[ngram]=value
...
if __name__ == '__main__':
    ...
    bigram_star1 = c_star(2,bigram_letter_counter1,bigram_witten_bell1,unigram_letter_counter1)
    bigram_star2 = c_star(2,bigram_letter_counter2,bigram_witten_bell2,unigram_letter_counter2)
    bigram_star3 = c_star(2,bigram_letter_counter3,bigram_witten_bell3,unigram_letter_counter3)
    bigram_star4 = c_star(2,bigram_letter_counter4,bigram_witten_bell4,unigram_letter_counter4)

    trigram_star1 = c_star(3,trigram_letter_counter1,trigram_witten_bell1,bigram_letter_counter1)
    trigram_star2 = c_star(3,trigram_letter_counter2,trigram_witten_bell2,bigram_letter_counter2)
    trigram_star3 = c_star(3,trigram_letter_counter3,trigram_witten_bell3,bigram_letter_counter3)
    trigram_star4 = c_star(3,trigram_letter_counter4,trigram_witten_bell4,bigram_letter_counter4)
...
##    print sorted(bigram_prob_tilde1.iteritems(), key=lambda (k,v):v,reverse=True)
...
##    print sorted(trigram_prob_tilde1.iteritems(), key=lambda (k,v):v,reverse=True)
##   
...
######################################## TEST PART ###########################################
    ...
    for ngram,value in bigram_letter_counter_test.iteritems():
        ...
        for word in ngram.split():
            actual_list[space+word+space]+=1
        result1=test_part(actual_list,order,trigram_backoff_prob1)
        result2=test_part(actual_list,order,trigram_backoff_prob2)
        result3=test_part(actual_list,order,trigram_backoff_prob3)
        result4=test_part(actual_list,order,trigram_backoff_prob4)
...
##    print sorted(trigram_backoff_prob1.iteritems(), key=lambda (k,v):v,reverse=True)